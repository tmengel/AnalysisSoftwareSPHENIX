    ...
    for(int ialgo=0;ialgo<_active_algo;ialgo++){
      ...
      for(Int_t iclus=0; iclus<(Int_t)_clusters_calo[ialgo][icalo].size(); iclus++){
        ...
        if ((_clusters_calo[ialgo][icalo].at(iclus)).cluster_isMatched){
          ...
          TVector3 recpartvec(m_tr_px[trackID],m_tr_py[trackID],m_tr_pz[trackID]);
          ...
          if ( (int)((_clusters_calo[ialgo][icalo].at(iclus)).cluster_matchedTrackIDs).size() > 1){
            for (Int_t k = 1; k < (int)((_clusters_calo[ialgo][icalo].at(iclus)).cluster_matchedTrackIDs).size(); k++){
              ...
              TVector3 recpartvec2(m_tr_px[trackIDAlt],m_tr_py[trackIDAlt],m_tr_pz[trackIDAlt]);
              if (int_CS_verbosity){
                if (k == 1){
