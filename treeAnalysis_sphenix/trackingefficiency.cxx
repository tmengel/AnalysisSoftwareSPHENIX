...
const unsigned int nTrackSources = 1;
...
void trackingefficiency(){
  ...
  for(Int_t itrk=0; itrk<(Int_t)m_nTracks; itrk++){
    unsigned short trackSource = 1;
    TVector3 recpartvec(m_tr_px[itrk],m_tr_py[itrk],m_tr_pz[itrk]);
    TVector3 mcpartvec(_mcpart_px[(int)m_truthtrackID[itrk]],_mcpart_py[(int)m_truthtrackID[itrk]],_mcpart_pz[(int)m_truthtrackID[itrk]]);
    // std::cout << itrk << std::endl;
    float receta = recpartvec.Eta();
    float trueeta = mcpartvec.Eta();
    if (verbosityTRKEFF) std::cout << "\tTRKTRUEID " << (int)m_truthtrackID[itrk] << "\tPDG: " << _mcpart_PDG[(int)m_truthtrackID[itrk]] << "\tpT: " <<   TMath::Sqrt(TMath::Power(m_tr_px[itrk],2)+TMath::Power(m_tr_py[itrk],2)) << "\tEta " << receta << std::endl;
    float pt = recpartvec.Pt();
    float truept = mcpartvec.Pt();
    float pmom = recpartvec.Mag();
    float truepmom = mcpartvec.Mag();
    TParticlePDG *part = TDatabasePDG::Instance()->GetParticle(abs(_mcpart_PDG[(int)m_truthtrackID[itrk]]));
    Double_t mass = part->Mass();
    float trueE     = TMath::Sqrt(truepmom*truepmom+mass*mass);
    for(int ipart=0;ipart<(int)nPart_TRKEFF;ipart++){
      if(abs(_mcpart_PDG[(int)m_truthtrackID[itrk]])==int_TRKEFF_mcparticles_PDG[ipart]){
...
//           std::cout << h_tracksTrue_Eta_pT[ipart][k]->GetName() << "\t" << h_tracksRec_Eta_pT[ipart][k]->GetName()  << "\t" << h_tracksTrue_Eta_p[ipart][k]->GetName() << "\t" << h_tracksRec_Eta_p[ipart][k]->GetName()<< std::endl;
  ...
  for(Int_t itrk=0; itrk<m_nTracks; itrk++){
    ...
    if (verbosityTRKEFF > 2) std::cout << "\tTRKTRUEID " << (int)m_truthtrackID[itrk] << "\tPDG: " << _mcpart_PDG[(int)m_truthtrackID[itrk]] << "\tpT: " <<   TMath::Sqrt(TMath::Power(m_tr_px[itrk],2)+TMath::Power(m_tr_py[itrk],2)) << "\tEta " << receta << std::endl;
    float pt        = recpartvec.Pt();
    float truept    = mcpartvec.Pt();
    float pmom      = recpartvec.Mag();
    float truepmom  = mcpartvec.Mag();
    ...
    Int_t parIdx = 5;
    if(abs(_mcpart_PDG[(int)m_truthtrackID[itrk]]) == 11)   parIdx = 0;
    if(abs(_mcpart_PDG[(int)m_truthtrackID[itrk]]) == 211)  parIdx = 1;
    if(abs(_mcpart_PDG[(int)m_truthtrackID[itrk]]) == 2212) parIdx = 2;
    if(abs(_mcpart_PDG[(int)m_truthtrackID[itrk]]) == 321)  parIdx = 3;
    if(abs(_mcpart_PDG[(int)m_truthtrackID[itrk]]) == 13)   parIdx = 4;
...
//   for (unsigned int iOuterTrk = 0; iOuterTrk < (unsigned int)m_nTracks; ++iOuterTrk) {
//     int iOuterTrkSource = _track_source[iOuterTrk];
//     if (verbosityTRKEFF > 3)std::cout << "outer track: "<< iOuterTrk << std::endl;
//     if ((int)m_truthtrackID[iOuterTrk] < 0) continue;
//     if (_mcpart_RecTrackIDs[(int)m_truthtrackID[iOuterTrk]].size() > 1){
//       for (int rtr = 0; rtr < (int)_mcpart_RecTrackIDs[(int)m_truthtrackID[iOuterTrk]].size(); rtr++){
//         int iInnerTrk = _mcpart_RecTrackIDs[(int)m_truthtrackID[iOuterTrk]].at(rtr);
//         int iInnerTrkSource = _track_source[iInnerTrk];
//         if ( _track_source[iOuterTrk] >= iInnerTrkSource ) continue;
//
//         TVector3 outerVec(m_tr_px[iOuterTrk], m_tr_py[iOuterTrk], m_tr_pz[iOuterTrk]);
//         TVector3 innerVec(m_tr_px[iInnerTrk], m_tr_py[iInnerTrk], m_tr_pz[iInnerTrk]);
